...
The repository contains four source areas:

- `src/managed/RTMaquetaXR`: managed C# game, UI, input, camera, and rendering integration.
- `src/launcher`: the Windows VR launcher source. It reuses the runtime, settings, and approved-default policies from the managed source tree.
...
The managed assembly targets the .NET/Mono environment used by the game and references assemblies from the user's game installation. Those assemblies are not redistributed. The launcher is a Windows x64 WinForms application built from the three C# files and `initial-settings.cfg` in `src/launcher`, together with `ApprovedLayout80.cs`, `ApprovedUserDefaults.cs`, `SettingsFiles76.cs`, `OpenXrRuntimePolicy.cs`, `RuntimeDiscovery80.cs`, and `RuntimeManifest80.cs` from the managed source tree. The settings file is embedded as `InitialDefaults80.cfg`. The native components require Visual Studio 2022 C++ tools, CMake, the Windows SDK, Khronos OpenXR SDK 1.1.63, and the relevant NVIDIA SDK headers and libraries. The OFXR provider additionally requires the upstream AMD FidelityFX SDK revision identified in its `THIRD_PARTY.md`.
