...
namespace RTMaquetaXR
{
    public static partial class Main
    {
        ...
        {
            var menu=ImageGroup("Mod menus and tutorial","Move this menu and its full help pages independently of the game panels. Placement is fitted to both eyes without replacing your saved offsets.",
                ImageValue("Mod menu horizontal position","Move the mod menu left or right. Zero requests its original horizontal position. The visible area can limit the applied position.",
                    ()=>(_cfg.modMenuOffsetX*100).ToString("0",ModLocalization.Culture)+"%",d=>{_cfg.modMenuOffsetX=PanelFit80.Offset(_cfg.modMenuOffsetX+d*.01f);MarkSettingsDirty();},()=> (_cfg.modMenuOffsetX+.65f)/1.3f),
                ImageValue("Mod menu height","Move the mod menu up or down. Zero requests its original height. The visible area can limit the applied position.",
                    ()=>(_cfg.modMenuOffsetY*100).ToString("0",ModLocalization.Culture)+"%",d=>{_cfg.modMenuOffsetY=PanelFit80.Offset(_cfg.modMenuOffsetY+d*.01f);MarkSettingsDirty();},()=> (_cfg.modMenuOffsetY+.65f)/1.3f),
                ImageValue("Mod menu distance","Move the mod menu and its tutorial nearer or farther. Both share horizontal position, height and distance.",
                    ()=>_cfg.modMenuDistance81.ToString("0.00",ModLocalization.Culture)+" m",d=>{_cfg.modMenuDistance81=Mathf.Clamp(_cfg.modMenuDistance81+d*.05f,.5f,3);MarkSettingsDirty();}),
            ...
            var group=ImageGroup("Interface position and size","Adjust HUD, management windows, maps, control hints and the mod menu independently. Changes are saved with your configuration.",
                ImageGroup("Game management screens","Position inventory and other management windows independently of the regular HUD.",
                ...
                menu,
                NativeGroupOptions81("Interactive dialogues",()=>_cfg.dialogLayout81),
                NativeGroupOptions81("Large game tutorials",()=>_cfg.largeTutorialLayout81),
                NativeGroupOptions81("Small game tutorials and tips",()=>_cfg.tipLayout81),
                ImageGroup("Other panels","Position HUD, maps and control hints.",
                    ...
                    ShareLayoutOption80(advanced,"Control hint placement")),
                new OverlayOption{Label="Restore factory placement",Description="Restore the approved interface layout only. Saved Custom and layout presets, world camera, quality and runtime remain unchanged.",
                    Menu=ConfirmationMenu("Restore factory placement","Replace the current interface placement with the factory layout?",()=>{ApprovedLayout80.CopyTo(_cfg,false);ResetInterfaceGroups81();ApplyHudPresetPlacement();_liveNavigation.OpenMenu(_qualityRoot);})});
