...
inline bool Valid(int runtime) { return runtime == 0 || runtime == 1 || runtime == 2; }
inline bool Matches(int runtime, std::string name) {
    ...
    if (runtime == 2) return name.find("pimax") != std::string::npos && name.find("pimaxxr") == std::string::npos;
