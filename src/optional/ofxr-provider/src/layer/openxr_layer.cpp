...
constexpr char kLayerName[] = "XR_APILAYER_RTMaquetaXR_ofxr77";
...
XrResult layer_release_swapchain_image_impl(
    ...
    if (state->session->manual_control.stop_requested() || !state->session->menu_enabled || (xrfg::host77::snapshot().flags & 1)==0) {
...
[[nodiscard]] bool capture_projection_snapshot(
    ...
    for (std::uint32_t layer_index = 0; layer_index < source->layerCount; ++layer_index) {
        ...
        if ((projection->layerFlags & XR_COMPOSITION_LAYER_BLEND_TEXTURE_SOURCE_ALPHA_BIT)!=0 ||
...
XrResult layer_end_frame_impl(
    ...
    const auto bypass_generation =
        [&](GenerationPrepareReason reason) -> XrResult {
            xrfg::host77::report(4,static_cast<std::uint32_t>(reason));
    ...
    xrfg::host77::report(pair_ready?3u:2u);
