...
struct D3D12FrameSynthesizer::Impl {
    ...
    void sync_diagnostics77() noexcept {
        const bool wanted=backend==D3D12OpticalFlowBackend::nvidia&&bridge_flight_logger().enabled();
        if(!wanted){nvidia_gpu_timing_enabled=false;for(auto& slot:work_slots)slot.timing_pending=false;return;}
        if(nvidia_gpu_timing_enabled)return;
        nvidia_gpu_timing_enabled=true;
        const UINT node=queue->GetDesc().NodeMask;
        if(!nvidia_timestamp_heap||!nvidia_timestamp_readback) {
            nvidia_timestamp_heap.Reset();nvidia_timestamp_readback.Reset();
            if(FAILED(create_nvidia_timing_resources(node))){nvidia_gpu_timing_enabled=false;return;}
        }
        for(auto& slot:work_slots) {
            slot.timing_pending=false;
            if(slot.timing_marker_command_list)continue;
            slot.timing_marker_allocator.Reset();
            HRESULT result=device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,IID_PPV_ARGS(slot.timing_marker_allocator.GetAddressOf()));
            if(SUCCEEDED(result))result=device->CreateCommandList(node,D3D12_COMMAND_LIST_TYPE_DIRECT,slot.timing_marker_allocator.Get(),nullptr,IID_PPV_ARGS(slot.timing_marker_command_list.GetAddressOf()));
            if(SUCCEEDED(result))result=slot.timing_marker_command_list->Close();
            if(FAILED(result)){slot.timing_marker_command_list.Reset();nvidia_gpu_timing_enabled=false;return;}
        }
    }
